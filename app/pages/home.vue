<template>
  <PageLayout>
      ...
      <div class="mb-6 text-left ml-1">
        <div class="flex items-center justify-between">
          ...
          >
            ...
            <span
              class="absolute top-1.5 right-1.5 block w-2.5 h-2.5 bg-red-500 rounded-full ring-2 ring-white"
            ></span>
          </button>
        </div>
      </div>
        ...
        >
          ...
          <div class="flex flex-1 items-center gap-3 group">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Cari program latihan, pemeriksaan, atau turnamen..."
                  ...
                  <div class="p-1">
                    ...
                    >
                      ...
                      <div class="mr-3 flex-shrink-0">
                        <svg
                          v-if="option.value === 'program-latihan'"
                          class="h-4 w-4"
                          fill="none"
                          stroke="currentColor"
                          stroke-width="2"
                        ...
                        <svg
                          v-else-if="option.value === 'pemeriksaan'"
                          class="h-4 w-4"
                          fill="none"
                          stroke="currentColor"
                          stroke-width="2"
                        ...
                        <svg
                          v-else-if="option.value === 'turnamen'"
                          class="h-4 w-4"
                          fill="none"
                          stroke="currentColor"
                          stroke-width="2"
    ...
    <div class="mb-6">
      <div
        class="rounded-2xl bg-white/70 backdrop-blur-md p-5 border border-white/40"
      >
        <!-- Top row with enhanced icons and animations -->
        <div class="flex justify-between">
          <div
            @click="navigateTo('/program-latihan')"
          ...
          >
            ...
            <span
              class="text-xs font-semibold text-gray-700 text-center group-hover:text-[#597BF9] transition-colors duration-200 whitespace-nowrap"
              >P. Latihan</span
            >
          </div>

          <div
            @click="navigateTo('/pemeriksaan')"
            class="flex flex-col items-center gap-2 group cursor-pointer"
          >
            <div
            ...
            <span
              class="text-xs font-semibold text-gray-700 text-center group-hover:text-green-600 transition-colors duration-200 whitespace-nowrap"
              >Pemeriksaan</span
            >
          </div>

          <div
            @click="navigateTo('/turnamen')"
            class="flex flex-col items-center gap-2 group cursor-pointer"
          >
            <div
            ...
            <span
              class="text-xs font-semibold text-gray-700 text-center group-hover:text-orange-600 transition-colors duration-200 whitespace-nowrap"
              >Turnamen</span
            >
          </div>
        </div>
    ...
    <div
      v-if="selectedModule === 'all' || selectedModule === 'program'"
      class="mb-6"
    >
      ...
      <!-- Loading State -->
      <div v-if="homeLoading" class="flex justify-center items-center py-8">
        <div
      ...
      >
        <p class="text-red-600 text-sm">{{ homeError }}</p>
        <button
          @click="fetchHomeData"
      ...
      <div class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
        <div
          v-for="program in filteredProgramLatihan"
          :key="program.id"
          @click="navigateToProgram(program.id)"
          class="flex-shrink-0 w-64 rounded-2xl bg-white/80 backdrop-blur-sm p-5 border border-white/40 hover:bg-blue-50 transition-all duration-300 cursor-pointer"
        >
          <!-- Image -->
          ...
          <h3 class="mb-4 text-sm font-bold text-gray-700 line-clamp-2">
            {{ program.nama }}
          </h3>
    ...
    <!-- Pemeriksaan -->
    <div
      v-if="selectedModule === 'all' || selectedModule === 'pemeriksaan'"
      class="mb-6"
    >
      <div class="mb-4 flex items-center justify-between">
      ...
      <div class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
        <div
          ...
          :key="pemeriksaan.id"
          @click="navigateToPemeriksaan(pemeriksaan.id)"
          class="flex-shrink-0 w-64 rounded-2xl bg-white/80 backdrop-blur-sm p-5 border border-white/40 hover:bg-green-50 transition-all duration-300 cursor-pointer"
        >
          <!-- Image -->
          ...
          <div class="mt-3 flex items-center gap-2 text-xs text-gray-600">
            ...
            <span class="font-medium">{{
              formatTanggal(pemeriksaan.tanggal)
            }}</span>
          </div>
        </div>
      </div>
    ...
    <!-- Turnamen -->
    <div
      v-if="selectedModule === 'all' || selectedModule === 'turnamen'"
      class="mb-8"
    >
      <div class="mb-4 flex items-center justify-between">
      ...
      <div class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
        <div
          ...
          :key="turnamen.id"
          @click="navigateToTurnamen(turnamen.id)"
          class="flex-shrink-0 w-64 rounded-2xl bg-white/80 backdrop-blur-sm p-5 border border-white/40 hover:bg-red-50 transition-all duration-300 cursor-pointer"
        >
          <!-- Image -->
          ...
          <div class="mt-3 flex items-center gap-2 text-xs text-gray-600">
            ...
            <span class="font-medium">{{
              formatPeriode(turnamen.periode)
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div
      v-if="
        !homeLoading &&
        !homeError &&
        filteredProgramLatihan.length === 0 &&
        filteredPemeriksaan.length === 0 &&
        filteredTurnamen.length === 0
      "
    ...
    >
      ...
      <p class="text-gray-500 mb-4">
        {{
          searchQuery || selectedCabor.length > 0 || selectedModule !== 'all'
      ...
      <button
        ...
        "
        @click="clearFilters"
        class="px-4 py-2 bg-[#597BF9] text-white rounded-lg text-sm font-medium hover:bg-[#4c6ef5] transition-colors"
      >
        Hapus Filter
      </button>
    </div>
...
const userName = computed(() => {
  console.log('userName computed - user.value:', user.value)
...
// Dropdown options - dynamic based on available data
const dropdownOptions = computed(() => {
  const options = [{ value: 'all', label: 'Semua Program' }]

  if (
    homeData.value?.programLatihan &&
    homeData.value.programLatihan.length > 0
  ) {
    options.push({ value: 'program', label: 'Program Latihan' })
  }

  if (homeData.value?.pemeriksaan && homeData.value.pemeriksaan.length > 0) {
    options.push({ value: 'pemeriksaan', label: 'Pemeriksaan' })
  }

  if (homeData.value?.turnamen && homeData.value.turnamen.length > 0) {
    options.push({ value: 'turnamen', label: 'Turnamen' })
  }

  return options
})

// Selected option based on selectedModule from composable
const selectedOption = computed(() => {
  const option = dropdownOptions.value.find(
    (opt) => opt.value === selectedModule.value
  )
  return option || dropdownOptions.value[0]
})

// Category filter options - dynamic based on available cabor
const categoryOptions = computed(() => {
  const options = [{ value: '', label: 'Semua Kategori' }]

  availableCabor.value.forEach((cabor) => {
    options.push({ value: cabor, label: cabor })
  })

  return options
})

// Selected category based on selectedCabor from composable
const selectedCategory = computed(() => {
  if (selectedCabor.value.length === 0) {
    return categoryOptions.value[0]
  }
  if (selectedCabor.value.length === 1) {
    const option = categoryOptions.value.find(
      (opt) => opt.value === selectedCabor.value[0]
    )
    return option || categoryOptions.value[0]
  }
  return {
    value: 'multiple',
    label: `${selectedCabor.value.length} kategori dipilih`,
  }
})
...
// Navigation functions for home data
const navigateToProgram = (id: number) => {
  router.push(`/program-latihan/${id}`)
}

const navigateToPemeriksaan = (id: number) => {
  router.push(`/pemeriksaan/${id}`)
}

const navigateToTurnamen = (id: number) => {
...
const selectCategory = (category: any) => {
  if (category.value === '') {
    // Clear all cabor filters
    selectedCabor.value = []
  } else {
    // Toggle cabor filter
    toggleCaborFilter(category.value)
  }
  isCategoryDropdownOpen.value = false
...
watch(searchQuery, () => {
  ...
  searchTimeout = setTimeout(() => {
    // Search is handled by computed properties, no need for additional action
...
onMounted(async () => {
  ...
  console.log('Home page loaded, user:', userName.value)
})
...
// Function to toggle save state (kept for future use)
const toggleSave = (category: string, id: number) => {
  console.log(`Toggle save for ${category} ${id}`)
