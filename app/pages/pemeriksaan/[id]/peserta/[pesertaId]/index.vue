<template>
  <PageLayout>
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <h1 class="text-xl font-bold text-gray-700">Pemeriksaan Peserta</h1>
    </div>
    ...
    >
      ...
      <div class="mt-3">
        <button
          @click="fetchPesertaParameterList()"
    ...
    <div v-else>
      ...
      <!-- Parameter List -->
      <div v-if="parameterList && parameterList.length > 0" class="space-y-4">
        <div
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div class="flex items-center gap-4 text-sm text-gray-600">
                <span>Parameter: {{ parameter.parameter }}</span>
                <span>•</span>
                <span>Nilai Terakhir: {{ parameter.nilaiTerakhir }}</span>
              </div>
              <div class="mt-2">
                <span
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium"
                  :class="getStatusClass(parameter.status)"
                >
                  {{ parameter.status }}
                </span>
              </div>
      ...
      <div v-else class="text-center py-12">
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-2">
          Tidak ada parameter pemeriksaan
        </h3>
        <p class="text-gray-500">
          Peserta ini belum memiliki parameter pemeriksaan yang tercatat.
        </p>
      </div>
    </div>
...
console.log('Pemeriksaan peserta page - Pemeriksaan ID:', pemeriksaanIdParam)
console.log('Pemeriksaan peserta page - Peserta ID:', pesertaIdParam)
...
// Navigation function
const viewGrafik = (parameterId: number) => {
  console.log('Navigating to grafik for parameter:', parameterId)
  router.push(
    `/pemeriksaan/${pemeriksaanId}/peserta/${pesertaId}/parameter/${parameterId}/grafik`
  )
}

onMounted(async () => {
  console.log('Pemeriksaan peserta page loaded for:', {
    pemeriksaanId,
    pesertaId,
  })
  await fetchPesertaParameterList()
})
</script>
